...
static void NDEF_FillVcardStruct( u8* pPayload, u32 PayloadSize, char* pKeyWord, u32 SizeOfKeyWord, u8* pString);
...
static void NDEF_ExtractVcard ( sRecordInfo *pRecordStruct, sVcardInfo *pVcardStruct )
{
	...
	NDEF_FillVcardStruct(pPayload , PayloadSize,VERSION, VERSION_STRING_SIZE, (u8*)(pVcardStruct->Version));
	if( !memcmp(pVcardStruct->Version, VCARD_VERSION_2_1, VCARD_VERSION_2_1_SIZE))
	{
		NDEF_FillVcardStruct(pPayload , PayloadSize,FIRSTNAME, FIRSTNAME_STRING_SIZE, (u8*)(pVcardStruct->FirstName));
		NDEF_FillVcardStruct(pPayload , PayloadSize,TITLE, TITLE_STRING_SIZE, (u8*)(pVcardStruct->Title));
		NDEF_FillVcardStruct(pPayload , PayloadSize,ORG, ORG_STRING_SIZE, (u8*)(pVcardStruct->Org));
		NDEF_FillVcardStruct(pPayload , PayloadSize,HOME_ADDRESS, HOME_ADDRESS_STRING_SIZE, (u8*)(pVcardStruct->HomeAddress));
		NDEF_FillVcardStruct(pPayload , PayloadSize,WORK_ADDRESS, WORK_ADDRESS_STRING_SIZE, (u8*)(pVcardStruct->WorkAddress));		
		NDEF_FillVcardStruct(pPayload , PayloadSize,HOME_TEL, HOME_TEL_STRING_SIZE, (u8*)(pVcardStruct->HomeTel));
		NDEF_FillVcardStruct(pPayload , PayloadSize,WORK_TEL, WORK_TEL_STRING_SIZE, (u8*)(pVcardStruct->WorkTel));		
		NDEF_FillVcardStruct(pPayload , PayloadSize,CELL_TEL, CELL_TEL_STRING_SIZE, (u8*)(pVcardStruct->CellTel));
		NDEF_FillVcardStruct(pPayload , PayloadSize,HOME_EMAIL, HOME_EMAIL_STRING_SIZE, (u8*)(pVcardStruct->HomeEmail));	
		NDEF_FillVcardStruct(pPayload , PayloadSize,WORK_EMAIL, WORK_EMAIL_STRING_SIZE, (u8*)(pVcardStruct->WorkEmail));		
	}
...
/**
  ...
  */
u16 NDEF_ReadVcard ( sRecordInfo *pRecordStruct, sVcardInfo *pVcardStruct )
{
	u16 status = ERROR;
...
/**
  ...
  */
u16 NDEF_WriteVcard ( sVcardInfo *pVcardStruct )
{
	u16 status = ERROR;
	u16 DataSize;
	u32 PayloadSize = 0;
...
/************************************/
	...
	DataSize = (u16)(PayloadSize); /* Must not count the 2 byte that represent the NDEF size */
	PayloadSize -= FIRST_RECORD_OFFSET+6+VCARD_TYPE_STRING_LENGTH;	
