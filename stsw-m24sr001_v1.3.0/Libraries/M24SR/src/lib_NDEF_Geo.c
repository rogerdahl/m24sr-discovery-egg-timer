...
static void NDEF_FillGeoStruct( u8* pPayload, u32 PayloadSize, sGeoInfo *pGeoStruct);
...
/**
  ...
  */
u16 NDEF_ReadGeo(sRecordInfo *pRecordStruct, sGeoInfo *pGeoStruct)
{
	u16 status = ERROR;
	uint16_t FileId;
...
/**
  ...
  */
u16 NDEF_WriteGeo ( sGeoInfo *pGeoStruct )
{
	u16 status = ERROR;
	u16 DataSize;
